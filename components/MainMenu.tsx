...
const MenuScene: React.FC = () => {
  return (
    <>
      ...
      {/* Floating Character */}
      {/* Reduced float intensity so it looks more grounded/idle */}
      <Float speed={2} rotationIntensity={0.1} floatIntensity={0.1}>
         <PlayerModel 
            position={{x:0, y:0, z:0}} 
            rotation={0} 
            animation="idle" // PlayerModel now handles finding the correct 'idle' clip
         />
      </Float>
    ...
    <div className="w-full h-full relative bg-gray-900">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-end pb-12 pointer-events-none">
        ...
        <div className="flex flex-col items-center gap-6 pointer-events-auto mb-8">
            ...
            <button 
                onClick={onPlay}
                className="bg-indigo-600 hover:bg-indigo-500 text-white text-2xl font-bold py-4 px-16 rounded-full shadow-[0_0_20px_rgba(79,70,229,0.5)] transition-all transform hover:scale-105 active:scale-95 border border-indigo-400"
            >
                PLAY
            </button>

            {/* Settings/Editor (Secondary) - Below Play */}
            <button 
                onClick={onEditor}
                className="flex items-center gap-2 bg-gray-800/80 hover:bg-gray-700 text-gray-300 py-2 px-6 rounded-full border border-gray-600 backdrop-blur-sm transition-all active:scale-95"
            >
